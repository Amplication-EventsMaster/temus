/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { CampaignsWhereUniqueInput } from "../../campaigns/base/CampaignsWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class CampaignsUpdateManyWithoutOrganizationsItemsInput {
  @Field(() => [CampaignsWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [CampaignsWhereUniqueInput],
  })
  connect?: Array<CampaignsWhereUniqueInput>;

  @Field(() => [CampaignsWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [CampaignsWhereUniqueInput],
  })
  disconnect?: Array<CampaignsWhereUniqueInput>;

  @Field(() => [CampaignsWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [CampaignsWhereUniqueInput],
  })
  set?: Array<CampaignsWhereUniqueInput>;
}

export { CampaignsUpdateManyWithoutOrganizationsItemsInput as CampaignsUpdateManyWithoutOrganizationsItemsInput };
